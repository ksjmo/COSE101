...
cal_day_t cal_first_day_of_month(int year, cal_month_t month) {
    ...
    for (cal_month_t cur_month = CAL_MONTH_JAN; cur_month < month; cur_month++) {
...
void cal_handle_print_command(int year, cal_month_t month, cal_event_t *events) {
...
void cal_handle_commands(void) {
    ...
    while (true) {
        ...
        if (!strcmp(command, "add")) {
            ...
            scanf("%i %i %i %i", &new_events->year, &new_events->month, &new_events->day, &new_events->type);
        ...
        if (!strcmp(command, "remove")) {
            ...
            while (true) {
                ...
                if (!ev->gone && ev->year == year && ev->month == month && ev->day == day) {
                    ev->gone = true;
                    break;
                }
                ev = ev->next;
        ...
        if (!strcmp(command, "save")) {
            ...
            while (ev != NULL) {
                if (!ev->gone) {
                    fprintf(save_file, "%i %i %i %i%s\n", ev->year, ev->month, ev->day, ev->type, ev->content);
                }
                ev = ev->next;
            }
            fclose(save_file);
        ...
        if (!strcmp(command, "load")) {
            ...
            while (!feof(in_file)) {
                ...
                new_events->next = events;
                new_events->gone = false;
                fscanf(in_file, "%i %i %i %i", &new_events->year, &new_events->month, &new_events->day, &new_events->type);
                for (int i = 0; i < 1000; i++) {
